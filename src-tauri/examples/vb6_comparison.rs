...
fn main() {
    ...
    println!("Location: Sukabumi P. Ratu ({:.6}°, {:.6}°)", location.latitude, location.longitude);
    println!("Date: {}-{:02}-{:02}", date.year, date.month, date.day as i32);
    println!();
    ...
    println!("  RA: {:.6}° = {}h", sun_pos.right_ascension, sun_pos.right_ascension / 15.0);
    ...
    println!("  RA: {:.6}° = {}h", moon_pos.right_ascension, moon_pos.right_ascension / 15.0);
    ...
    let cos_sep = dec1_rad.sin() * dec2_rad.sin() + dec1_rad.cos() * dec2_rad.cos() * ra_diff_rad.cos();
    let angular_sep = cos_sep.acos().to_degrees();
    println!("Angular separation (Geocentric RA/Dec): {:.6}°", angular_sep);
    ...
    let (moon_topo_ra, moon_topo_dec) = astronomy::topocentric::moon_topocentric_ra_dec(&location, sunset_jd);
    ...
    println!("Elongation: {} ({:.3}° diff)", if elongation_ok { "✓ PASS" } else { "✗ FAIL" }, (elongation - 1.316).abs());
    println!("Altitude: {} ({:.3}° diff)", if altitude_ok { "✓ PASS" } else { "✗ FAIL" }, (altitude - (-1.117)).abs());
    println!();
