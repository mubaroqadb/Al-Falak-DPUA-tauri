...
fn main() {
    ...
    println!("Difference from VB6: {:.8} days\n", (sunset18_jd - 2461089.970208).abs());
    ...
    println!("   Error: {:.4} jam ({:.1} menit)", (age18-21.957).abs(), (age18-21.957).abs()*60.0);
    ...
    println!("   Error: {:.4}° ({:.1} arcsec)", (elong18-11.096).abs(), (elong18-11.096).abs()*3600.0);
    println!("   Kriteria MABIMS: ≥ 3°");
    println!("   Status: {}\n", if elong18 >= 3.0 { "✅ PASS" } else { "❌ FAIL" });
    
    println!("3. TINGGI HILAL (Altitude) - Airy Refraction");
    println!("   VB6:   8.653° (8° 39' 11\")");
    println!("   Tauri: {:.3}°", alt18);
    println!("   Error: {:.4}° ({:.1} arcsec)", (alt18-8.653).abs(), (alt18-8.653).abs()*3600.0);
    ...
    println!("   RA:  {:.3}° (VB6: 343.021°) - Error: {:.3}°", moon_geo.right_ascension, (moon_geo.right_ascension - 343.021).abs());
    println!("   Dec: {:.3}° (VB6: -6.939°) - Error: {:.3}°", moon_geo.declination, (moon_geo.declination + 6.939).abs());
    println!("   Lon: {:.3}° (VB6: 341.699°) - Error: {:.3}°\n", moon_geo.longitude, (moon_geo.longitude - 341.699).abs());
    
    println!("Sun Position (Geocentric):");
    println!("   RA:  {:.3}° (VB6: 330.943°) - Error: {:.3}°", sun_geo.right_ascension, (sun_geo.right_ascension - 330.943).abs());
    println!("   Dec: {:.3}° (VB6: -11.890°) - Error: {:.3}°\n", sun_geo.declination, (sun_geo.declination + 11.890).abs());
    ...
    println!("Age error:      {:.1} menit - {} {}", (age18-21.957).abs()*60.0, if age_ok { "✅" } else { "⚠️" }, if age_ok { "(< 12 min)" } else { "(> 12 min)" });
    println!("Elongation err: {:.1} arcsec - {} {}", (elong18-11.096).abs()*3600.0, if elong_ok { "✅" } else { "⚠️" }, if elong_ok { "(< 12 as)" } else { "(> 12 as)" });
    println!("Altitude err:   {:.1} arcsec - {} {}", (alt18-8.653).abs()*3600.0, if alt_ok { "✅" } else { "⚠️" }, if alt_ok { "(< 12 as)" } else { "(> 12 as)" });
    println!();
    ...
    if mabims_pass {
        ...
        if age18 < 8.0 { println!("   • Umur Bulan: {:.3}h < 8h ❌", age18); }
        if elong18 < 3.0 { println!("   • Elongasi:   {:.3}° < 3° ❌", elong18); }
        if alt18 < 2.0 { println!("   • Tinggi:     {:.3}° < 2° ❌", alt18); }
