...
use crate::{Degrees, GeoLocation, JulianDay};
...
pub fn evaluate_criteria(
    ...
    conjunction_jd: JulianDay,
    observation_jd: JulianDay,
) -> MabimsResult {
    // Hitung umur bulan dalam jam
    let moon_age = (observation_jd - conjunction_jd) * 24.0;

    println!(
        "🔍 MABIMS eval - Conjunction JD: {}, Observation JD: {}, Age: {} h",
        conjunction_jd, observation_jd, moon_age
    );

    // Log exact JD being sent to function
    eprintln!(
        "⚡ MABIMS: About to call moon_altitude_topocentric with JD = {}",
        observation_jd
    );

    // Gunakan topocentric altitude calculation dari astronomy::topocentric module
    let moon_alt =
        crate::astronomy::topocentric::moon_altitude_topocentric(location, observation_jd);
...
/// Evaluasi kriteria visibilitas MABIMS Baru (3-6.4)
/// Kriteria MABIMS Baru (Imkan Rukyat): altitude >= 3°, elongation >= 6.4°
/// Note: Moon age is not a primary criteria in the new definition, but we calculate it for reference.
pub fn evaluate_new_mabims(
    location: &GeoLocation,
    conjunction_jd: JulianDay,
    observation_jd: JulianDay,
) -> MabimsResult {
    ...
    // Gunakan topocentric altitude calculation
    let moon_alt =
        crate::astronomy::topocentric::moon_altitude_topocentric(location, observation_jd);

    // Hitung elongasi geosentris (MABIMS pakai geosentris)
    let moon_pos = crate::astronomy::moon_position(observation_jd);
    let sun_pos = crate::astronomy::sun_position(observation_jd);
    ...
    // Evaluasi kriteria MABIMS Baru: altitude >= 3, elongation >= 6.4
    let is_visible = moon_alt >= 3.0 && elongation >= 6.4;
