    ...
    if (!result) {
      content.innerHTML = `
        <div class="no-results flex flex-col items-center justify-center p-8 opacity-60">
          ...
          <p class="font-bold">${this.t('results.noResults', 'No calculation results available')}</p>
          <p class="text-xs">${this.t('results.selectLocationCriteria', 'Please select a location and criteria, then click calculate.')}</p>
        </div>
      `;
      return;
    }
    ...
    if (result.error) {
      content.innerHTML = `
        <div class="error-results flex flex-col items-center justify-center p-8 text-error bg-error/10 rounded-xl">
          ...
          <p class="font-bold">${this.t('results.calculationError', 'Calculation Error')}</p>
